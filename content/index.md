...
Some time ago I was asked what could be done to reduce cost pressure, ease the maintenance burden on our spatial teams, address the data silos that have grown across the department, improve access security, and make spatial data delivery faster and more reliable.

This letter introduces my answer.

The work is a cloud-native spatial platform reference architecture. In simpler terms, it is a complete design for a modern platform that can store, manage, secure, serve, and edit spatial data in a more consistent way across the department.
...
The artefact is a nineteen-chapter design corpus, available as a navigable site and exported into our Confluence space. It is written to stand on its own. The prototype that informed it is not being handed over as a production system. It was valuable as a way to test ideas, but it was not production-reviewed and should not be treated as a deployable asset. What is being handed over is the design, the reasoning behind it, the options that were tested and rejected, and the lessons learned along the way.
...
Instead, I spent time in conversation with senior stakeholders, developed a view about the kind of spatial platform the department is likely to need over the longer term, and then built enough of that view to test whether it was realistic.

That approach was deliberate. The department has had many opportunities for incremental improvement. What it has not had is a clear, worked-through position on what a simpler, lower-overhead, more secure, and more durable spatial platform could be.

This work is offered as that position. It is not a demand that the department immediately change direction. It is a stake in the ground that future investment, procurement, and architecture decisions can be measured against.
...
Adopting the design across the department would require investment. A full transition would likely need capital funding in the order of $2–3 million. That would cover production hardening, migration planning and tooling, support for existing project teams, technical assurance, and the safety nets needed for a responsible transition.
...
There are three realistic ways the department could use this architecture.
...
The third is to use it as a reference point. Even if the department does not adopt the architecture directly, the design decisions, lessons learned, and platform comparisons can help shape future procurement, investment, and technical choices. It provides a defensible view of what good could look like in this domain, and a useful counterpoint when assessing vendor proposals or incremental uplift options.

## Closing

I have worked hard on this, and I have worked differently. I took responsibility for forming a view, testing it, documenting it, and leaving behind something that others could use.

I am sorry that I will not be here to see what happens with it. The work has mattered to me, and so have the people I have worked with while doing it. I hope the technical architecture is useful, whether it becomes a platform direction, a source of reusable components, or simply a reference point for better decisions in the future.
