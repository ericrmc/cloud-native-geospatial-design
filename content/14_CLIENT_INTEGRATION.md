...
## Migrating from ArcGIS Enterprise

This section is indicative — enough to scope a migration, not a runbook. Each ArcGIS Enterprise capability is mapped to its closest equivalent on this platform; in most cases the migration is a parallel-run rather than a cutover, with the platform standing up alongside the existing Portal and individual services moving once their consumers are pointed at the new endpoints.

| ArcGIS Enterprise capability | This platform's equivalent | Migration approach |
|---|---|---|
| Feature services (hosted feature layers) | OGC API Features over GeoParquet (see [06 OGC Features API](06_OGC_FEATURES_API.md)) | Export the layer to GeoParquet (or GeoJSON for small layers), upload through the reviewed editing pipeline, repoint clients at the new `/features/{collection}` URL. |
| Vector tile services | PMTiles served via `/tiles/vector/{dataset}/{z}/{x}/{y}.mvt` (see [05 Vector Tiles](05_VECTOR_TILES.md)); a dedicated Esri VectorTileServer adapter exposes the same layers in the shape ArcGIS Enterprise Portal expects | For browser maps, point any MapLibre / Leaflet / OpenLayers client at the new tile URL. For ArcGIS-resident consumers (Pro, Online, embedded Portal viewers), register the adapter URL as a vector tile service in the new Portal or as an external item. |
| Map services (rendered raster tiles from operational layers) | Generate vector tiles for vector layers (preferred — client-side styling), or run the raster tile path for genuinely rasterised content (see [08 Raster Services](08_RASTER_SERVICES.md)) | Most "map services" exist because the originating data was vector and Server-side rendering was the only option; migrate those to PMTiles and shift the styling to the client. Reserve raster tile generation for hillshades, basemaps, and pre-rendered cartographic products. |
| Image services (raster and imagery) | Raster tiles via TiTiler + WMTS/WMS proxy; OGC API Coverages for scientific/continuous data | Convert source rasters to Cloud-Optimised GeoTIFF (single `gdal_translate -of COG` per file), register a MosaicJSON pointing at them, and the platform serves WMTS/WMS/Coverages over the same imagery without copying it again. |
| Symbology and rendering | Client-side: MapLibre style JSON, ArcGIS web map JSON via the VectorTileServer adapter | Existing renderers do not port directly. The expected path is to author MapLibre styles for the first-party client; consumers using ArcGIS may keep their existing rendering by binding the adapter's tile service into their existing Portal items. |
| Web maps and web apps (Portal items) | First-party web map client (see [15 Map Client](15_MAP_CLIENT.md)) plus the catalogue surface (see [10 Discovery](10_DISCOVERY.md)) | Catalogue records replace Portal items as the index. The first-party client renders most browse-and-edit workflows; dashboards or bespoke viewers can be rebuilt against the standard interfaces. |
| Sharing and groups (Portal sharing model) | Platform groups, dataset-level grants, public-access flags (see [03 Authorisation](03_AUTHORISATION.md)) | Portal groups become platform groups; Portal item permissions become per-dataset grants; "everyone" / "everyone in organisation" map to the platform's public flag and an organisation-wide group respectively. |
| Editing (feature service editing, attachments, versioning) | Reviewed editing pipeline (see [11 Editing Pipeline](11_EDITING_PIPELINE.md)) | The biggest workflow shift. ArcGIS edits are typically live transactional; the platform's edits are reviewed-batch. For teams that need live editing, the platform is not a like-for-like replacement and a parallel solution should be retained. For teams whose ArcGIS edit workflows already involve review (most large authoritative datasets do), the platform's flow is a closer fit. |
| Geoprocessing services (server-published Python toolboxes) | Lambda for short, request-shaped jobs; Fargate tasks orchestrated by Step Functions for long-running batch work (see [11 Editing Pipeline](11_EDITING_PIPELINE.md) and [12 Deployment](12_DEPLOYMENT.md)) | Existing Python toolboxes can usually be lifted into one of these shapes with modest changes — replace ArcPy with `geopandas` / `rasterio` / `pdal` for spatial operations, swap inputs from feature-service URLs to platform tile/feature endpoints. The platform does not currently provide a managed geoprocessing surface; the recommendation is to run analytical work as containers or Lambdas that consume the platform's data, not as platform-hosted toolboxes. |
| Network analysis (Network Analyst) | Routing service (Valhalla via the query layer — see [09 Routing](09_ROUTING.md)) | Map the operations: route → route, isochrone → isochrone, OD-cost matrix → matrix. Custom network datasets (private road networks) require building a custom Valhalla profile and graph; for most public-road use this is not needed. |
| Print services (export web map to PDF) | Adjacent — no first-class print service (see [19 Business Requirements §9](19_BUSINESS_REQUIREMENTS.md#9-reporting-and-exports)) | If print is a hard requirement, keep an ArcGIS print service in parallel, or stand up a headless renderer (the PDF-report sketch in [17 Further Directions](17_FURTHER_DIRECTIONS.md)). |
| Hosted notebooks (ArcGIS Notebooks) | Adjacent — not provided | Notebook users connect to the platform's data via standard HTTP + GeoPandas / DuckDB from any Jupyter environment (local, SageMaker, Databricks). |

**Migration order, in practice:**

1. Stand the platform up alongside ArcGIS Enterprise in a sandbox; load one or two non-critical datasets end to end (upload → tile → serve → consume from QGIS and ArcGIS Pro).
2. Onboard ArcGIS Enterprise users to the platform's authorisation model — federate the same identity provider, recreate the group structure.
3. Migrate read-only consumers first (vector tiles, raster tiles, OGC Features) — these are low-risk and immediately demonstrate cost and performance differences.
4. Migrate reviewed-editing workflows next; keep live-editing workflows on ArcGIS until the team decides whether to change process or retain a parallel system.
5. Port geoprocessing toolboxes one at a time, each as either a Lambda or a Fargate task. Retire the corresponding ArcGIS geoprocessing service only once consumers are repointed.
6. Decommission ArcGIS services in the reverse order they were onboarded — read-only consumers last, ensuring the rendered URL endpoints remain available throughout the cutover window.
