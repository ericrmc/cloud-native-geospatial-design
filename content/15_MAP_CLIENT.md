...
> *In plain terms:* the "live" feel is bought with a short polling loop and the tile server's automatic ETag refresh, not with a persistent socket connection. That keeps every component stateless and cheap-when-idle — there is no socket server to keep warm.
