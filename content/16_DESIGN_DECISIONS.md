...
| **[GeoServer Cloud](https://github.com/geoserver/geoserver-cloud)** | Cloud-native refactor of GeoServer into Spring Boot microservices, intended for ECS/Kubernetes deployment. Distinct from standalone GeoServer. | **Trialled in earnest.** Got it running on ECS after non-trivial configuration. Three concrete blockers in practice: (1) could not be made to read COGs from a private S3 bucket (public buckets worked); (2) the SOAP/XML configuration API was difficult to drive to generate correct mosaic descriptors; (3) the admin UI was awkward to work with for the workflows we needed. One genuine strength: the security layer is granular and well-conceived. Net judgment: the custom orchestration and adaptation work required to make it serve the platform's specific needs outweighed writing the smaller custom code path for the narrower scope this platform actually uses. Worth re-evaluating as the project matures and the S3 / mosaic-generation gaps close. |
